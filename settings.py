...
from microphone_manager import DEFAULT_DEVICE_LABEL, input_device_names
...
class SettingsEditor:
    def __init__(self, root: tk.Tk, config_path: str | Path) -> None:
        ...
        self.loaded_window_state = self._window_state(self.config)
        ...
        self.pil_cache: dict[str, tuple[int, Image.Image]] = {}
    ...
    def _build_glow_tab(self, parent: ttk.Frame) -> None:
        ...
        for index, key in enumerate(KEYS):
            ...
            ttk.Button(
                parent,
                text=KEY_LABELS[key],
                command=lambda value=key: self._select_key(value),
    ...
    def _build_mouse_glow_tab(self, parent: ttk.Frame) -> None:
        ...
        for index, button in enumerate(("left", "right")):
            ttk.Button(
                ...
                command=lambda value=button: self._select_mouse_button(value),
            ).grid(
                row=0,
                column=index * 2,
                columnspan=2,
    ...
    def _build_glow_geometry_fields(
        ...
        for index, (label, variable) in enumerate(zip(labels, self._glow_variables(mode))):
    ...
    def _build_motion_tab(self, parent: ttk.Frame) -> None:
        ...
        for label, variable in (
            ...
            command = (
                ...
                if variable is self.transparent_background
                else None
            )
            ttk.Checkbutton(
                parent, text=label, variable=variable, command=command
            ).grid(
    ...
    def _commit_image_fields(self, show_error: bool = True) -> bool:
        try:
            x = float(self.image_x.get())
            y = float(self.image_y.get())
            width = max(0.0, float(self.image_width.get()))
            height = max(0.0, float(self.image_height.get()))
        except ValueError:
            if show_error:
    ...
    def _load_glow_geometry_fields(self, mode: str) -> None:
        ...
        for variable, value in zip(
            self._glow_variables(mode), (*spec["position"], *spec["size"])
        ):
    ...
    def _commit_glow_geometry_fields(self, mode: str, show_error: bool) -> bool:
        try:
            x, y, width, height = (
                float(variable.get()) for variable in self._glow_variables(mode)
            )
        except ValueError:
            if show_error:
    ...
    def _collect_general_fields(self) -> None:
        try:
            ...
            self.config["right_hand_speed"] = float(self.right_hand_speed.get())
            range_x = max(0.0, float(self.right_hand_range_x.get()))
            range_y = max(0.0, float(self.right_hand_range_y.get()))
    ...
    def _save(self) -> None:
        if (
            not self._commit_image_fields()
            or not self._commit_glow_fields()
            or not self._commit_mouse_glow_fields()
        ):
            return
        try:
            self._collect_general_fields()
            self._collect_microphone_fields()
            editor_config = self.config

            def merge_editor_config(latest_config: dict[str, Any]) -> dict[str, Any]:
                for name, original_value in self.loaded_window_state.items():
                    if editor_config[name] == original_value:
                        editor_config[name] = latest_config[name]
    ...
    @staticmethod
    def _window_state(config: dict[str, Any]) -> dict[str, Any]:
        return {
            "window_x": config["window_x"],
            "window_y": config["window_y"],
            "window_position_locked": config["window_position_locked"],
    ...
    def _read_pil_image(self, spec: dict[str, Any]) -> Image.Image | None:
        ...
        try:
            ...
            return None
        cache_key = str(path)
        cached = self.pil_cache.get(cache_key)
        if cached is not None and cached[0] == modified:
            ...
            try:
                ...
                return None
            self.pil_cache[cache_key] = (modified, source)
        return self._resize_pil(source, spec)

    @staticmethod
    def _resize_pil(source: Image.Image, spec: dict[str, Any]) -> Image.Image:
    ...
    def _preview_image_size(
        ...
        image = self._read_pil_image(spec)
        if image is not None:
            return image.size
        return {
            "character": (300, 300),
            "desk_keyboard": (300, 300),
            "right_hand": (300, 300),
            "right_hand_mouse": (300, 300),
            "left_hand": (300, 300),
            "left_hand_idle": (300, 300),
            "left_hand_pressed": (300, 300),
        }.get(name, (100, 100))
    ...
    def _draw_layer(
        ...
        actual_position = position if position is not None else spec["position"]
        x, y = round(actual_position[0]), round(actual_position[1])
        image = self._read_pil_image(spec)
        if image is not None:
            composition.alpha_composite(image, (x, y))
            return
        width, height = self._preview_image_size(name, spec)
        draw = ImageDraw.Draw(composition, "RGBA")
        draw.rectangle(
            (x, y, x + width, y + height),
            fill=(45, 45, 45, 150),
            outline=(240, 240, 240, 210),
            width=2,
        )
    ...
    def _draw_glow(
        ...
        for step in range(12, 0, -1):
            fraction = step / 12
            half_width = width * fraction / 2
            half_height = height * fraction / 2
    ...
    def _drag_preview(self, event: tk.Event[Any]) -> None:
        ...
        for variable, value in zip(self._glow_variables(self.drag_target), new_position):
            variable.set(self._format_number(value))
        self._redraw_preview()
...
def main(arguments: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="iram_tap 설정")
